...
local Window = Rayfield:CreateWindow({
   Name = "⚡ Hub Definitivo | Mega Multi-Jogos",
   LoadingTitle = "Carregando Módulos Expandidos...",
...
-- ABAS DO MENU
local TabGames    = Window:CreateTab("🎮 Outros Jogos", 4483362458)
...
-- ==================== ABA OUTROS JOGOS (EXPANDIDA) ====================

TabGames:CreateParagraph({Title = "Módulos Avançados por Jogo", Content = "Ative funções específicas de acordo com o jogo em que você está."})

-- Murder Mystery 2
TabGames:CreateButton({
   Name = "🔪 Murder Mystery 2: Puxar Arma Caída",
   Callback = function()
      local found = false
...
-- Blade Ball
TabGames:CreateButton({
   Name = "⚔️ Blade Ball: Modo Reação Rápida (FOV Assist)",
   Callback = function()
      FOVRadius = 250
      FOVCircle.Radius = 250
      ShowFOVCircle = true
      FOVCircle.Visible = true
      Rayfield:Notify({Title = "Blade Ball", Content = "Área de foco da bola expandida!", Duration = 3})
   end,
})

-- Pet Simulator / Simulators
TabGames:CreateToggle({
   Name = "🐾 Pet Sim / Simulators: Auto-Coleta de Moedas/Ovos",
   CurrentValue = false,
      ...
      task.spawn(function()
         while _G.AutoFarmSims do
            ...
            for _, item in pairs(workspace:GetDescendants()) do
               ...
               if string.find(string.lower(item.Name), "coin") or string.find(string.lower(item.Name), "gem") or string.find(string.lower(item.Name), "egg") or string.find(string.lower(item.Name), "chest") then
...
-- Arsenal / FPS Games
TabGames:CreateButton({
   Name = "🔫 Arsenal / FPS: Otimizar Mira e Remover Recuo Visual",
   Callback = function()
      for _, v in pairs(Camera:GetDescendants()) do
         if v:IsA("CameraShaker") then v:Destroy() end
      end
      Rayfield:Notify({Title = "FPS Games", Content = "Efeitos de tremor removidos!", Duration = 3})
   end,
})

-- Brookhaven RP
TabGames:CreateButton({
   Name = "🏡 Brookhaven RP: Modo Fantasma (Invisibilidade Leve)",
   Callback = function()
...
TabGames:CreateButton({
   ...
   Callback = function()
      ...
      Rayfield:Notify({Title = "Doors", Content = "Iluminação máxima aplicada!", Duration = 3})
   end,
})

-- Blox Fruits
TabGames:CreateButton({
   Name = "🍎 Blox Fruits: Ativar Auto-Clicker de Combate",
   Callback = function()
      AutoClickerEnabled = not AutoClickerEnabled
      Rayfield:Notify({Title = "Blox Fruits", Content = "Auto-Click alterado para: " .. tostring(AutoClickerEnabled), Duration = 3})
   end,
})
...
TabGames:CreateButton({
   ...
   Callback = function()
      if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Humanoid") then
         ...
         Rayfield:Notify({Title = "BedWars", Content = "Velocidade ajustada para combate!", Duration = 3})
      end
   end,
})
...
Rayfield:Notify({Title = "⚡ Sucesso", Content = "Hub Mega Multi-Jogos Carregado!", Duration = 5})
